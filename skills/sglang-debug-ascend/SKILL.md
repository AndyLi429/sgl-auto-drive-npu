---
name: ascend-910c
description: SSH into the Ascend 910C node at <your-npu-host-ip>, enter Docker container `<your-container-name>`, work in `/home/<your-user>/sglang`, and use the ready 910C remote environment for SGLang development and validation. Use when a task needs remote NPU work, HCCL-aware tests, multi-NPU smoke tests, graph-mode checks, or a safe remote copy instead of local-only execution. Trigger this whenever the user asks to run, test, validate, benchmark, or smoke-test SGLang code on Ascend 910C / NPU hardware — even if they don't say "910C" explicitly. Also trigger for phrases like "在我的设备上跑", "远端跑一下", "在NPU上验证", "在910C上跑", "用我的开发机", "on my NPU box", "on the Ascend node", or any reference to the configured container or host IP.
---
...
Use this skill to do SGLang development on the Ascend 910C node through `<your-npu-host-ip>`.
The default container is `<your-container-name>` and the repo lives at `/home/<your-user>/sglang`.
...
- container `<your-container-name>` is running on an SGLang-Ascend image
- the repo is cloned at `/home/<your-user>/sglang`
...
> ```
> Host ascend_910c
>     HostName <your-npu-host-ip>
>     User <your_user>
> ```
> Then replace every `<ssh_alias>` below with `ascend_910c`. Until you do, fall back to `ssh <user>@<your-npu-host-ip>`.
...
ssh <ssh_alias> 'docker exec <your-container-name> npu-smi info'
```

2. Enter the container and repo.

```bash
ssh <ssh_alias> 'docker exec -it <your-container-name> /bin/bash'
cd /home/<your-user>/sglang
...
```bash
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "npu-smi info | head -40"'
```

For a single-line summary across all 8 NPUs:

```bash
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "for i in 0 1 2 3 4 5 6 7; do npu-smi info -t usages -i \$i -c 0 2>/dev/null | head -5 ; echo --- ; done"'
```
...
```bash
ssh <ssh_alias> 'docker start <your-container-name>'
```
...
```bash
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "cd /home/<your-user>/sglang && git branch --show-current && git status --short"'
```

2. Fast-forward `/home/<your-user>/sglang` to the latest clean `main` before creating any validation worktree.

```bash
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "cd /home/<your-user>/sglang && git fetch origin && git checkout main && git pull --ff-only origin main"'
```

3. Avoid writing directly into `/home/<your-user>/sglang` when it is dirty or when the local snapshot differs from the remote `HEAD`.
...
```bash
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "cd /home/<your-user>/sglang && git worktree add --detach /tmp/sglang_validate_npu HEAD"'
```
...
COPYFILE_DISABLE=1 tar --exclude=.git -cf - . | \
  ssh <ssh_alias> 'docker exec -i <your-container-name> sh -lc "rm -rf /tmp/sglang_local_validate && mkdir -p /tmp/sglang_local_validate && tar -xf - -C /tmp/sglang_local_validate"'
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "find /tmp/sglang_local_validate -name '\''._*'\'' -delete"'
```
...
That keeps `/home/<your-user>/sglang` clean while still validating the exact local delta.
...
```bash
ssh <ssh_alias> 'docker exec <your-container-name> bash -lc "cd /tmp/sglang_local_validate && python -m compileall python/sglang"'
```

2. Run targeted tests for the changed area.

```bash
ssh <ssh_alias> 'docker exec <your-container-name> env PYTHONPATH=python bash -lc "cd /tmp/sglang_local_validate && pytest -q path/to/test.py -q"'
```

3. For NPU-backed changes, pin a free NPU explicitly with `ASCEND_RT_VISIBLE_DEVICES`.

```bash
ssh <ssh_alias> 'docker exec <your-container-name> env ASCEND_RT_VISIBLE_DEVICES=0 PYTHONPATH=python bash -lc "cd /tmp/sglang_local_validate && pytest -q path/to/npu_test.py -q"'
```

4. For multi-NPU / HCCL work, set a visible device list and use the standard launch flow. Example for TP=4:

```bash
ssh <ssh_alias> 'docker exec <your-container-name> env ASCEND_RT_VISIBLE_DEVICES=0,1,2,3 PYTHONPATH=python bash -lc "cd /tmp/sglang_local_validate && python -m sglang.launch_server --model-path <model_path> --tp 4 --device npu --port 30000"'
```
...
```bash
ssh <ssh_alias> 'docker exec <your-container-name> env ASCEND_RT_VISIBLE_DEVICES=0 PYTHONPATH=python bash -lc "cd /tmp/sglang_local_validate && python <your_profiler_script>.py"'
...
```bash
ssh <ssh_alias> 'docker exec <your-container-name> rm -rf /tmp/sglang_local_validate /tmp/sglang_validate_npu'
```